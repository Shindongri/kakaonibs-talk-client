...
const requestSignIn = function*({ payload }: any) {
  try {
    const {
      ...
      data: {
        statusText,
        user: { userName, uuid, isLogged },
      },
    } = yield call(() => axios.post('/user/signin', payload))
...
const requestSignOut = function*({ payload }: any) {
  try {
    const {
      status,
      data: { statusText },
    } = yield call(() => axios.post(`/user/signout`, payload))
...
export default function* roomSaga() {
  yield all([takeLatest([REQUEST_SIGNIN], requestSignIn), takeLatest([REQUEST_SIGNOUT], requestSignOut)])
}
